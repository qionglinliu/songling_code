#!/usr/bin/env python3
"""
Piper数据采集脚本 v1 - LeRobot格式
基于 recorder.py 修改：action 的夹爪使用当前帧 master 的夹爪值

数据格式:
  - observation.state = puppet位置 (当前状态)
  - action = 下一帧puppet位置 + 当前帧master夹爪
...
使用方法:
  ...
  python recorder_v1.py --repo-id . --arm left  # 追加模式
"""
...
class PiperRecorderV1:
    """Piper数据采集器 v1 - action夹爪用master值"""
    ...
    def __init__(
        ...
        self.master_left_deque = deque(maxlen=2000)   # 新增: master 左臂
        self.master_right_deque = deque(maxlen=2000)  # 新增: master 右臂
    ...
    def _init_ros(self):
        """初始化ROS订阅"""
        rospy.init_node('piper_recorder_v1', anonymous=True)
        ...
        # master 订阅 (新增)
        if self.is_left_arm or self.is_dual_arm:
            rospy.Subscriber('/master/joint_left', JointState, 
                             self._cb_master_left, queue_size=1000, tcp_nodelay=True)
        if self.is_right_arm or self.is_dual_arm:
            rospy.Subscriber('/master/joint_right', JointState, 
                             self._cb_master_right, queue_size=1000, tcp_nodelay=True)
        ...
        rospy.loginfo(f"ROS订阅已初始化 - {arm_type}模式 (v1: action夹爪用master)")
    ...
    def _cb_master_left(self, msg: JointState):
        self.master_left_deque.append(msg)
    
    def _cb_master_right(self, msg: JointState):
    ...
    def _get_frame(self) -> dict | None:
        ...
        if self.is_right_arm or self.is_dual_arm:
            ...
            if len(self.puppet_right_deque) == 0:
                return None
            if len(self.master_right_deque) == 0:
                return None
        ...
        if self.is_left_arm or self.is_dual_arm:
            ...
            timestamps.append(self.master_left_deque[-1].header.stamp.to_sec())
        if self.is_right_arm or self.is_dual_arm:
            ...
            timestamps.append(self.master_right_deque[-1].header.stamp.to_sec())
        ...
        if self.is_left_arm or self.is_dual_arm:
            ...
            while self.master_left_deque[0].header.stamp.to_sec() < frame_time:
                self.master_left_deque.popleft()
            master_left = self.master_left_deque.popleft()
        ...
        if self.is_right_arm or self.is_dual_arm:
            ...
            while self.master_right_deque[0].header.stamp.to_sec() < frame_time:
                self.master_right_deque.popleft()
            master_right = self.master_right_deque.popleft()
        ...
        if self.is_dual_arm:
            ...
            # master夹爪
            master_gripper_left = master_left.position[6]
            master_gripper_right = master_right.position[6]
        elif self.is_left_arm:
        ...
        # 保存 master 夹爪值
        if self.is_dual_arm:
            frame_data["master_gripper"] = (master_gripper_left, master_gripper_right)
        elif self.is_left_arm:
            frame_data["master_gripper"] = master_gripper_left
        else:
            frame_data["master_gripper"] = master_gripper_right
    ...
    def record_episode(self, task: str = "演示任务") -> int:
        ...
        if len(raw_frames) > 1:
            ...
            for i in range(len(raw_frames) - 1):
                ...
                # action = 下一帧puppet位置 + 当前帧master夹爪
                action = next_frame["observation.state"].copy()
                
                if self.is_dual_arm:
                    # 左臂夹爪用当前帧master左臂夹爪
                    action[6] = current_frame["master_gripper"][0]
                    # 右臂夹爪用当前帧master右臂夹爪
                    action[13] = current_frame["master_gripper"][1]
                elif self.is_left_arm:
                    action[6] = current_frame["master_gripper"]
                else:
                    action[6] = current_frame["master_gripper"]
    ...
    def run(self, num_episodes: int = 10, task: str = "机器人操作演示"):
        ...
        print(f"  action = 下一帧puppet位置 + 当前帧master夹爪")
        print(f"\n操作说明:")
        print(f"  - 按回车键开始/停止录制")
        print(f"  - 按 q 键退出整个采集")
        print("="*50)
...
def main():
    parser = argparse.ArgumentParser(description="Piper数据采集 v1 (action夹爪用master)")
    ...
    parser.add_argument("--num-episodes", type=int, default=50, help="采集episode数量")
